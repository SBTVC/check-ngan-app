...
export function UserProfileSummary({ role, compact = false }: { role: Role; compact?: boolean }) {
  ...
  const name = user?.fullName || user?.firstName || (role === 'teacher' ? 'คุณครู' : 'นักเรียน')
  const email = user?.primaryEmailAddress?.emailAddress ?? null
  const studentCode = role === 'student'
    ? firstString(metadata, ['studentId', 'student_id', 'studentCode', 'student_code', 'code'])
    : null
  const level = firstString(metadata, ['level', 'educationLevel', 'education_level', 'grade'])
  const className = firstString(metadata, ['className', 'class_name', 'classroom', 'room'])
  const department = firstString(metadata, ['department', 'departmentName', 'department_name'])
  const major = firstString(metadata, ['major', 'majorName', 'major_name', 'branch', 'program'])
  const position = role === 'teacher' ? firstString(metadata, ['position', 'title', 'teacherPosition']) : null
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
      <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex min-w-0 items-center gap-4">
          <Avatar name={name} imageUrl={user?.imageUrl} size="lg" />
          <div className="min-w-0">
        ...
        {extra.length > 0 ? (
          ...
          <p className="max-w-sm text-xs leading-5 text-slate-400 sm:text-right">แสดงเฉพาะข้อมูลบัญชีที่มีอยู่จริงใน Clerk เท่านั้น</p>
        )}
      </div>
    </section>
  )
